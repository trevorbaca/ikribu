...
def bcl(m):
    ...
    for pair in [(2, 4), (10, 12), (15, 17)]:
        with baca.scope(m.get(pair)) as o:
            baca.hairpinlib.exact(o[:2], "p<fff")
            baca.hairpinlib.exact(o[-1:], "fff>o!", rleak=True)
    with baca.scope(m.get(23, 31)) as o:
        baca.hairpinlib.exact(o[:-1], "ppp<fff")
...
def strings(cache):
    for name in ["vn", "va", "vc"]:
        for pair in [(2, 4), (10, 12), (15, 17), (23, 27)]:
            baca.hairpinlib.exact(cache[name].get(pair), "mf<fff")
        if name in ("vn", "vc"):
